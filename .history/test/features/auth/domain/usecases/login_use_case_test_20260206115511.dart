...
void main() {
  ...
  const tUser = UserEntity(id: 1, ssoId: 1, username: tEmail, name: 'Test User', role: 1, shouldChangePassword: false);

  test('should return UserEntity when login is successful', () async {
    // Arrange
    when(
      () => mockAuthRepository.login(username: tEmail, password: tPassword),
    ).thenAnswer((_) async => const Right(tUser));

    // Act
    final result = await useCase.execute(username: tEmail, password: tPassword);
