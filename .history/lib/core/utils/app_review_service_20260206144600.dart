...
class AppReviewServiceImpl implements AppReviewService {
  ...
  static const String _keyLastReviewRequestTime = 'app_review_last_request_time';
  ...
  Future<void> init() async {
    ...
    if (!_preferences.containsKey(_keyFirstLaunchTime)) {
      await _preferences.setInt(_keyFirstLaunchTime, DateTime.now().millisecondsSinceEpoch);
    }
  ...
  Future<bool> shouldRequestReview() async {
    ...
    final firstLaunchTime = _preferences.getInt(_keyFirstLaunchTime) ?? DateTime.now().millisecondsSinceEpoch;
    final lastReviewRequestTime = _preferences.getInt(_keyLastReviewRequestTime) ?? 0;

    // Calculate days since first launch
    final daysSinceFirstLaunch = DateTime.now().difference(DateTime.fromMillisecondsSinceEpoch(firstLaunchTime)).inDays;

    // Calculate days since last review request
    final daysSinceLastReviewRequest = DateTime.now().difference(DateTime.fromMillisecondsSinceEpoch(lastReviewRequestTime)).inDays;

    // Check if we've waited long enough since the last request
    if (lastReviewRequestTime > 0 && daysSinceLastReviewRequest < _daysBetweenReviewRequests) {
      return false;
    }

    // Check thresholds
    return sessionCount >= _minSessionsBeforeReview && daysSinceFirstLaunch >= _minDaysBeforeReview && actionCount >= _minActionsBeforeReview;
  }
  ...
  Future<void> requestReview() async {
    try {
      ...
      if (await _inAppReview.isAvailable()) {
        ...
        await _preferences.setInt(_keyLastReviewRequestTime, DateTime.now().millisecondsSinceEpoch);
      } else {
  ...
  Future<void> recordSignificantAction() async {
    ...
    if (await shouldRequestReview()) await requestReview();
  }

  @override
  Future<bool> showFeedbackForm({required BuildContext context, required String title, required String message}) async {
