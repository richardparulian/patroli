...
class BasicUpdateService implements UpdateService {
  ...
  BasicUpdateService({
    ...
    Future<UpdateInfo?> Function(String currentVersion)? updateInfoResolver,
  }) : _androidPackageName = androidPackageName,
       _iOSAppId = iOSAppId,
       _updateInfoResolver = updateInfoResolver;
  ...
  Future<UpdateInfo> _fetchUpdateInfo() async {
    ...
    final resolver = _updateInfoResolver;
    if (resolver == null) {
      throw StateError('No update info resolver configured.');
    }

    final updateInfo = await resolver(currentVersion);
    if (updateInfo == null) {
      throw StateError('Update info resolver returned no data.');
    }
