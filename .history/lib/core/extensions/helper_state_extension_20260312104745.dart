...
extension ResultStateX<T> on ResultState<T> {
  ...
  bool get isSuccess => this is Success<T> && !(this as Success<T>).isError;

  bool get isError => this is Error<T> && !(this as Error<T>).isError;
...
extension ResultStateSingleX<T> on ResultState<T> {
  T? get successData => this is Success<T> ? (this as Success<T>).data : null;
