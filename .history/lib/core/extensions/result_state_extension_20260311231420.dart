...
class Success<T> extends ResultState<T> {
  ...
  const Success(this.data, {this.consumed = false});

  Success<T> copyWith({bool? consumed}) {
    return Success(data, consumed: consumed ?? this.consumed);
  }
}
...
class Error<T> extends ResultState<T> {
  ...
  const Error(this.message, {this.consumed = false});

  Error<T> copyWith({bool? consumed}) {
    return Error(message, consumed: consumed ?? this.consumed);
  }
}
