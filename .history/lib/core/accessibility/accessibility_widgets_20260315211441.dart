...
const int _kTooltipDurationSeconds = 5;
...
extension AccessibilityWidgetExtensions on Widget {
  ...
  Widget withAccessibleTooltip(String message) {
    return Tooltip(
      message: message,
      showDuration: const Duration(seconds: _kTooltipDurationSeconds),
      child: this,
    );
  }
  ...
  Widget conditionallyAccessible(
    ...
    return Builder(
      builder: (context) {
        final isScreenReaderActive =
            MediaQuery.maybeOf(context)?.accessibleNavigation ?? false;

        if (isScreenReaderActive && isAccessible) {
          return Semantics(
