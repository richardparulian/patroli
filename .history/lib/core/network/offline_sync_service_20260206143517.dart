...
class OfflineChange {
  ...
  factory OfflineChange.fromJson(Map<String, dynamic> json) {
    return OfflineChange(
      ...
      lastRetryTime: json['lastRetryTime'] != null ? DateTime.parse(json['lastRetryTime']) : null,
      errorMessage: json['errorMessage'],
    );
  }

  /// Create a copy with updated fields
  OfflineChange copyWith({String? id, String? entityType, String? entityId, OfflineOperationType? operationType, Map<String, dynamic>? data, SyncStatus? status, DateTime? timestamp, int? retryCount, DateTime? lastRetryTime, String? errorMessage}) {
    return OfflineChange(
      id: id ?? this.id,
...
abstract class ConflictResolutionStrategy {
  ...
  Future<Map<String, dynamic>> resolveConflict({required String entityType, required String entityId, required Map<String, dynamic>? localData, required Map<String, dynamic>? remoteData, required OfflineOperationType operationType});
}
...
class ClientWinsStrategy implements ConflictResolutionStrategy {
  @override
  Future<Map<String, dynamic>> resolveConflict({required String entityType, required String entityId, required Map<String, dynamic>? localData, required Map<String, dynamic>? remoteData, required OfflineOperationType operationType}) async {
...
abstract class OfflineSyncService {
  ...
  Future<OfflineChange> queueChange({required String entityType, String? entityId, required OfflineOperationType operationType, Map<String, dynamic>? data});
  ...
  Future<void> resolveConflict(String changeId, Map<String, dynamic> resolvedData);
...
class HiveOfflineSyncService implements OfflineSyncService {
  ...
  }) : _box = box, _connectivity = connectivity;
  ...
  @override
  Future<OfflineChange> queueChange({required String entityType, String? entityId, required OfflineOperationType operationType, Map<String, dynamic>? data}) async {
    final id = _uuid.v4();
    ...
    await _syncLock.synchronized(() async {
      ...
      try {
        ...
        final pendingChanges = await getPendingChanges().then(
          (changes) => changes.where((c) => c.status == SyncStatus.pending).toList(),
        );
  ...
  Future<SyncStatus?> getSyncStatus(String entityType, String entityId) async {
    ...
    final entityChanges = changes.where((c) => c.entityType == entityType && c.entityId == entityId).toList();
  ...
  @override
  Future<void> resolveConflict(String changeId, Map<String, dynamic> resolvedData) async {
...
class StreamController<T> {
  ...
  /// Get the stream
  Stream<T> get stream => Stream<T>.periodic(const Duration(days: 365), (_) {
    throw UnimplementedError('This is a mock stream for demo purposes');
  }).asBroadcastStream()..listen((event) {}, onDone: () {}, onError: (error, stack) {});
}
