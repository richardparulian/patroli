...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    final collapseOffset = ScreenUtil.sh(_appBarExpandedHeight) - kToolbarHeight;
    final opacity = (notification.metrics.pixels / collapseOffset).clamp(0.0, 1.0);

    if ((opacity - _appBarBackgroundOpacity).abs() > 0.01) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final appBarBackgroundColor = color.primary.withValues(alpha: _appBarBackgroundOpacity);
