...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final appBarBackgroundColor = Color.lerp(Colors.transparent, color.primary, _scrollProgress);
    final appBarForegroundColor = Color.lerp(color.onPrimary, color.onPrimary, _scrollProgress) ?? color.onPrimary;
    final headerTitleOpacity = (1 - (_scrollProgress * 1.2)).clamp(0.0, 1.0);
    final headerSubtitleOpacity = (1 - (_scrollProgress * 1.8)).clamp(0.0, 1.0);
        ...
        child: RefreshIndicator(
          ...
          child: NotificationListener<ScrollNotification>(
            onNotification: (notification) {
              final progress = (notification.metrics.pixels / 24).clamp(0.0, 1.0);
              if ((progress - _scrollProgress).abs() > 0.95) {
                setState(() {
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        CircleAvatar(
                          ...
                          child: Opacity(
                            opacity: 0.92 + (_scrollProgress * 0.08),
                            child: ClipOval(
                              child: CircleImages(
                          ...
                          children: [
                            Text(context.tr('security_patrol_system'),
                              style: theme.textTheme.titleMedium?.copyWith(
                                color: color.onPrimary.withValues(alpha: headerTitleOpacity),
                            ...
                            Text(context.tr('branch_control_reporting'),
                              style: theme.textTheme.bodyMedium?.copyWith(
                                color: color.onPrimary.withValues(alpha: 0.8 * headerSubtitleOpacity),
                              ),
                            ),
                          ],
                        ),
