...
import 'package:pos/features/reports/data/models/reports_model.dart';
...
class ReportsRepositoryImpl implements ReportsRepository {
  ...
  @override
  Future<Either<Failure, List<ReportsEntity>>> getReportss() async {
    try {
      final models = await _remoteDataSource.fetchReportss();
  ...
  @override
  Future<Either<Failure, ReportsEntity>> getReports(int id) async {
    try {
      final model = await _remoteDataSource.fetchReports(id);
      return Right(model.toEntity());
    } on ServerException {
      return Left(ServerFailure(message: 'Failed to fetch reports'));
    } on NetworkException {
      return Left(NetworkFailure(message: 'No internet connection'));
    } on Exception catch (e) {
      return Left(ServerFailure(message: e.toString()));
