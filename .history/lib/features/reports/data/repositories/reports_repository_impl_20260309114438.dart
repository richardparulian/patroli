...
class ReportsRepositoryImpl implements ReportsRepository {
  ...
  @override
  Future<Either<Failure, List<ReportsEntity>>> getReports(int page, int limit) async {
    try {
      final models = await _remoteDataSource.fetchReports(page, limit);
  ...
  @override
  Future<Either<Failure, ReportsEntity>> countReports(int id) async {
    try {
      final model = await _remoteDataSource.countReports(id);
      return Right(model.toEntity());
    } on ServerException {
      return Left(ServerFailure(message: 'Failed to count reports'));
    } on NetworkException {
      return Left(NetworkFailure(message: 'No internet connection'));
    } on Exception catch (e) {
      return Left(ServerFailure(message: e.toString()));
