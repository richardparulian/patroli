...
class ReportsRepositoryImpl implements ReportsRepository {
  ...
  Future<Either<Failure, List<ReportsEntity>>> getReports({required int page, required int limit}) async {
    try {
      final result = await _remoteDataSource.fetchReports(page: page, limit: limit);
  ...
  @override
  Future<Either<Failure, int>> countReports({required int pagination}) async {
    try {
      final result = await _remoteDataSource.countReports(pagination: pagination);
      return Right(result);
    } on ServerException {
      rethrow;
    } on NetworkException {
