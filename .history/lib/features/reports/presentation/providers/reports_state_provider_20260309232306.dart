...
class ReportsNotifier extends Notifier<ReportsState> {
  ...
  void onSuccessReports(List<ReportsEntity> reports, {bool isLoadMore = false}) {
    final updatedReports = isLoadMore ? [...state.reports, ...newReports] : newReports;
  
    final hasReachedMax = newReports.length < 10; // Asumsi limit = 10
    state = state.copyWith(
      isLoading: false,
      isError: false,
      isSuccess: true,
      ...
      countByStatus: updatedReports.where((r) => r.statusValue == 1).length,
