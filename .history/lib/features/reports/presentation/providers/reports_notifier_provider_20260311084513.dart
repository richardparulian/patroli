...
class ReportsNotifier extends AsyncNotifier<ReportsState> {
  ...
  Future<List<ReportsEntity>> _fetchReports({int? page, int? limit, int? pagination}) async {
    final reportsUseCase = ref.read(reportsUseCaseProvider);
  ...
  Future<void> loadMore({required int page}) async {
    ...
    final newReports = await _fetchReports(page: page, limit: 10);

    final updatedReports = [...currentState.reports, ...newReports];
    final hasReachedMax = newReports.length < 10;
  ...
  void setCarouselIndex(int reportId, int index) {
    ...
    final updated = Map<int, int>.from(currentState.carouselIndexes);
    updated[reportId] = index;

    state = AsyncData(
      currentState.copyWith(
        carouselIndexes: updated,
      ),
    );
  }

  void clear() {
    state = const AsyncData(ReportsState());
  }
}
