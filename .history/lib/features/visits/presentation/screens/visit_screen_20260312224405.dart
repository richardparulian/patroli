...
class VisitScreen extends ConsumerStatefulWidget {
  ...
  const VisitScreen({super.key, required this.scanQr, required this.checkIn, required this.report});
...
class _VisitScreenState extends ConsumerState<VisitScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      final args = GoRouterState.of(context).extra as VisitRouteArgs?;

      final scanQrData = args?.scanQr;
      final reportData = args?.report;

      await ref.read(visitAttentionProvider.notifier).runVisitAttention(scanQrData?.qrcode ?? reportData?.branch?.qrcode ?? '');
    });
  }
          ...
          AppDialog.showSuccess(
            ...
            onButtonPressed: () {
              ...
              context.push(AppConstants.checkOutRoute, extra: {
                'reportId': value.id ?? reportData?.id ?? 0,
                'branchId': scanQrData?.id ?? reportData?.branch?.id ?? 0,
                'branchName': scanQrData?.name ?? reportData?.branch?.name ?? '-',
              });
            },
          );
        },
        ...
        appBar: AppBar(
          ...
          bottom: PreferredSize(
            ...
            child: Container(
              ...
              child: AppAlertCard(
                title: 'Cabang',
                message: scanQrData?.name ?? reportData?.branch?.name ?? '---',
                ...
                AppDialog.showConfirm(
                  ...
                  onConfirm: () async {
                    await ref.read(visitCreateProvider.notifier).runVisitCreate(
                      ...
                      reportId: checkInData?.id ?? reportData?.id ?? 0,
                    );
                  },
                );
              },
