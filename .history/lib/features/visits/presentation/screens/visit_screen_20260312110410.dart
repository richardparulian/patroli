...
class _VisitScreenState extends ConsumerState<VisitScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final isLoadingAttention = ref.watch(visitCreateProvider.select((s) => s.isLoading));
  ...
  Widget _buildFormSection({required bool isLoadingCheckOut}) {
    ...
    return ref.watch(visitAttentionProvider).when(
      idle: () => Center(child: Text('Menunggu data...')),
          ...
          children: [
            if (scanQrData.visitAttention?.requireAttentions == 1) ...[
              ...
              AppIconButton(
                ...
                onPressed: isLoadingCheckOut ? null : () async {
            ...
            // Sisanya AppRadioGroup, AppCheckboxGroup, AppTextField tetap seperti sebelumnya,
            // tapi ganti semua `visitData?.visitAttention` jadi `visitData.visitAttention`
          ],
        ),
      ),
  ...
  //   final visitData = ref.watch(visitAttentionProvider).dataOrNull;
