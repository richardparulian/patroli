...
class _ScanQrScreenState extends ConsumerState<ScanQrScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    _scanQrListener = ref.listenManual(scanQrStateProvider, (prev, next) {
      if (next.isSuccess && next.entity != null && mounted) { 
        ...
        ref.read(scanQrStateProvider.notifier).setProcessing(false);
  ...
  Future<void> _checkCameraPermission() async { 
    ...
    debugPrint('isCheckingPermission: ${state.isCheckingPermission}');

    if (state.isCheckingPermission) return;

    notifier.setCheckingPermission(true);
    ...
    if (!mounted) {
      notifier.setCheckingPermission(false);
      return;
    }

    if (hasPermission) {
      await ref.read(scanQrStateProvider.notifier).startScanner();
    }
  ...
  Future<void> _rebuildScanner() async {
    ...
    ref.read(scanQrStateProvider.notifier).setController(newController);
  ...
  Widget _buildScanner() {
    ...
    final isError = ref.watch(scanQrStateProvider.select((s) => s.isError));
    final isLoading = ref.watch(scanQrStateProvider.select((s) => s.isLoading));
    final isProcessing = ref.watch(scanQrStateProvider.select((s) => s.isProcessing));
    ...
    return AiBarcodeScanner(
      ...
      onDetect: (value) {
        ...
        if (value.barcodes.isNotEmpty) {
          ref.read(scanQrStateProvider.notifier).setProcessing(true);
