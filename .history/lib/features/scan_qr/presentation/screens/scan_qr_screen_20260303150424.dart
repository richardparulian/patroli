...
class _ScanQrScreenState extends ConsumerState<ScanQrScreen> with WidgetsBindingObserver {
  ...
  Future<void> _checkCameraPermission() async {
    ...
    _requestCameraPermission(status);
  }

  Future<void> _requestCameraPermission(PermissionStatus status) async {
    ...
    if (status == PermissionStatus.permanentlyDenied) {
      if (mounted) {
        AppDialog.showWarning(
          ...
          message: 'Izinkan akses ke kamera untuk mengambil foto',
          buttonText: 'Izinkan',
      ...
      body: AiBarcodeScanner(
        appBarBuilder: (context, controller) {
          return AppBar(
            ...
            leading: IconButton(
              icon: Icon(Iconsax.arrow_left, color: colorScheme.onPrimary),
