...
import 'package:pos/core/ui/toasts/app_toast.dart';
...
class _ScanQrScreenState extends ConsumerState<ScanQrScreen> with WidgetsBindingObserver {
  ...
  Future<void> _pickImageFromGallery() async {
    final state = ref.read(scanQrStateProvider.notifier);
    ...
    if (result != null && result.barcodes.isNotEmpty) {
      ...
      //   context: context,
      //   title: 'Gagal memindai QR Code',
      //   description: 'QR Code tidak ditemukan pada gambar!',
      //   onDismiss: () async {
      ...
      // );
      _controller.stop();
      state.setError('QR Code tidak ditemukan pada gambar!');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Stack(
        children: [
          Positioned.fill(
            child: state.errorMessage == null ? _buildScanner(colorScheme, state) : _buildError(colorScheme, state),
          ),
          ...
          Positioned(
            ...
            left: 0,
            right: 0,
            child: _buildBottomMenu(colorScheme, state),
          ),
        ],
      ),
    );
  ...
  Widget _buildScanner(ColorScheme colorScheme, ScanQrState state) {
  ...
  Widget _buildError(ColorScheme colorScheme, ScanQrState state) {
    return Container(
      child: Center(
  ...
  Widget _buildBottomMenu(ColorScheme colorScheme, ScanQrState state) {
    return Container(
              ...
              child: Row(
                ...
                children: [
                  Container(
                    ...
                    child: Icon(Iconsax.info_circle, color: colorScheme.primary, size: 28),
                  ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text('Kendala Scan QR Code?',
                          style: theme.textTheme.titleMedium?.copyWith(
                            fontWeight: FontWeight.bold,
                            color: colorScheme.onSurface,
                          ),
                        ),
                        Text('Silakan unggah QR Code dari album atau masukan kode cabang pada menu dibawah ini.',
