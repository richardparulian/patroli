...
class _ScanQrScreenState extends ConsumerState<ScanQrScreen> {
  ...
  Future<void> _onScanned(String qrCode) async {
    ...
    state.when(
      ...
      error: (error, _) {
        // ❌ ERROR: Show snackbar/dialog dan RESUME scanner
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Gagal: $error')),
          );
          _controller.resume(); // Resume scanner untuk scan lagi
        }
      },
    );
  }
      ...
      body: Stack(
        children: [
          AiBarcodeScanner(
            ...
            onDetect: (val) {
              // Filter hanya QR code yang valid
              if (val.isNotEmpty && !scanState.isLoading) {
                _onScanned(val);
              }
            },
            onDispose: () {
