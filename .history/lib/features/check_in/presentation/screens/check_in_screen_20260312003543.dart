...
class _CheckInScreenState extends ConsumerState<CheckInScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _initCamera() async {
    ref.read(cameraProvider.notifier).setInitializing(true);
  ...
  Future<void> _onCapture() async {
    ...
    await _animationController.forward();
      ...
      final scanQr = GoRouterState.of(context).extra as ScanQrEntity;
  ...
  Widget build(BuildContext context) {
    ...
    if (scanQrData == null) {
      return _buildErrorWidget('Data cabang tidak ditemukan', () => context.go(AppConstants.homeRoute));
    }
        ...
        body: Stack(
          children: [
            Positioned.fill(
              ...
              child: _buildSelfieSection(scanQrData),
            ),
  ...
  Widget _buildErrorWidget(String message, VoidCallback onButtonPressed) {
    final theme = Theme.of(context);
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          child: Icon(Iconsax.shop, size: 48, color: colorScheme.primary),
        ),
  ...
  Widget _buildSelfieSection(ScanQrEntity scanQrData) {
    final initializedState = ref.watch(checkInProvider.select((s) => s.isInitialized));
    final isInitialized = _cameraController?.value.isInitialized ?? false;
  ...
  Widget _buildButton(ScanQrEntity scanQrData) {
    final theme = Theme.of(context);
    final colorScheme = theme.colorScheme;
