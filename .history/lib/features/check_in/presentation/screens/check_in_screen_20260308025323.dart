...
class _CheckInScreenState extends ConsumerState<CheckInScreen> with SingleTickerProviderStateMixin {
  ...
  void _showErrorDialog() {
    ...
    final checkInState = ref.read(checkInStateProvider);
    
    if (!mounted) return;
    if (checkInState.errorMessage == null) return;
    ...
    AppDialog.showError(
      ...
      buttonText: 'Oke',
      onButtonPressed: () => checkInNotifier.clear(),
    );
  }

  Future<void> _initCamera() async {
    final checkInNotifier = ref.read(checkInStateProvider.notifier);
    checkInNotifier.setCameraInitializing(true);
    ...
    try {
      ...
      if (!hasPermission) {
        checkInNotifier.setCameraInitError('Izin kamera tidak diberikan');
        return;
      }
      ...
      if (cameras.isEmpty) {
        checkInNotifier.setCameraInitError('Tidak ada kamera yang tersedia');
        return;
      }
      ...
      checkInNotifier.setCameraInitializing(false);
      setState(() {});
      ...
      checkInNotifier.setCameraInitError('Gagal menginisialisasi kamera: ${e.toString()}');
  ...
  Widget build(BuildContext context) {
    ...
    debugPrint('Build: isLoading=${checkInState.isLoading}, isInitializingCamera=${checkInState.isInitializingCamera}');
                ...
                child: Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(checkInState.loadingMessage,
  ...
  Widget _buildSelfieSection(ScanQrEntity branchData) {
    ...
    if (checkInState.isInitializingCamera) {
      return Center(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
    ...
    if (checkInState.cameraInitError != null) {
      return _buildErrorWidget(
        Iconsax.camera_slash,
        checkInState.cameraInitError!,
        'Coba lagi',
        Iconsax.refresh,
        () async => await _initCamera(),
      );
    }

    if (!isInitialized && checkInState.cameraInitError == null) {
      return _buildErrorWidget(
        Iconsax.camera_slash,
