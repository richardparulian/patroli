...
class _CheckInScreenState extends ConsumerState<CheckInScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    if (branchData == null) {
      return _buildErrorWidget('Data cabang tidak ditemukan', 'Kembali ke Beranda', Iconsax.home, Iconsax.shop, () => context.go(AppConstants.homeRoute));
    }
  ...
  Widget _buildErrorWidget(String message, String buttonText, IconData iconButton, IconData iconMessage, VoidCallback onButtonPressed) {
    final theme = Theme.of(context);
    final colorScheme = theme.colorScheme;
  ...
  Widget _buildSelfieSection(ScanQrEntity branchData) {
    ...
    if (!isInitialized) {
      return _buildErrorWidget('Kamera gagal diinisialisasi', 'Coba lagi', Iconsax.refresh, Iconsax.camera_slash, () async => await _initCamera());
    }
