...
import 'package:pos/features/pre_sign/presentation/controllers/pre_sign_controller.dart';
...
class _CheckInScreenState extends ConsumerState<CheckInScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _onCapture() async {
    ...
    final preSignController = ref.read(preSignProvider.notifier);
    ...
    await preSignController.createPreSign(
      request: PreSignRequest(filename: uniqueFilename),
    );

    final preSignState = ref.read(preSignProvider);
  
    if (preSignState.hasValue && preSignState.value != null) {
      final preSignEntity = preSignState.value!;
      debugPrint('preSignEntity asd: $preSignEntity');
      await _handleUploadAndCheckIn(preSignEntity, image);
    }
  }

  Future<void> _handleUploadAndCheckIn(PreSignEntity preSignEntity, XFile image) async {
    try {
      ...
      AppDialog.showSuccess(
        ...
        onButtonPressed: () => context.go(AppConstants.homeRoute),
      );
    } catch (e) {
      if (!mounted) return;
      AppDialog.showError(
  ...
  Future<void> _uploadImage(XFile image, String uploadUrl) async {
    ...
    if (preSignState.hasValue && preSignState.value != null) {
      final preSignEntity = preSignState.value?.fileUrl;
      
      debugPrint('preSignEntity: $preSignEntity');
    }
  }
