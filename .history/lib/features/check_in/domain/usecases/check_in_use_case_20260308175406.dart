...
class CreateCheckInUseCase implements UseCase<CheckInEntity, CreateCheckInParams> {
  ...
  Future<Either<Failure, CheckInEntity>> call(CreateCheckInParams params) {
    ...
    if (params.request.branchId == 0) {
      return Future.value(
        Left(InputFailure(message: 'Maaf, cabang tidak valid. Silakan coba lagi. Jika masalah tetap terjadi, silakan hubungi admin. Terima kasih.'),
      );
    }

    if (params.request.selfieCheckIn.isEmpty) {
      return Future.value(
        Left(InputFailure(message: 'Maaf, foto selfie tidak valid')),
      );
    }

    if (params.request.branchId == 0 && params.request.selfieCheckIn.isEmpty) {
      return Future.value(
        Left(InputFailure(message: 'Maaf, cabang dan foto selfie tidak valid')),
      );
    }
