...
class CreateCheckInUseCase implements UseCase<CheckInEntity, CreateCheckInParams> {
  ...
  Future<Either<Failure, CheckInEntity>> call(CreateCheckInParams params) {
    ...
    if (params.request.branchId == 0) {
      return Future.value(
        Left(InputFailure(message: 'Maaf, cabang tidak ditemukan, silahkan coba lagi')),
      );
    }

    if (params.request.selfieCheckIn.isEmpty) {
      return Future.value(
        Left(InputFailure(message: 'Maaf, foto selfie tidak ditemukan, silahkan coba lagi')),
      );
    }

    if (params.request.branchId == 0 && params.request.selfieCheckIn.isEmpty) {
      return Future.value(
        Left(InputFailure(message: 'Maaf, cabang dan foto selfie tidak ditemukan, silahkan coba lagi')),
      );
    }
