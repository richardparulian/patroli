...
class CheckInRepositoryImpl implements CheckInRepository {
  ...
  @override
  Future<Either<Failure, List<CheckInEntity>>> getCheckIns() async {
    try {
      final models = await _remoteDataSource.fetchCheckIns();
      return Right(models.map((model) => model.toEntity()).toList());
    } on ServerException {
      return Left(ServerFailure(message: 'Failed to fetch check_ins'));
    } on NetworkException {
  ...
  @override
  Future<Either<Failure, CheckInEntity>> getCheckIn(int id) async {
    try {
      final model = await _remoteDataSource.fetchCheckIn(id);
...
final checkInRepositoryProvider =
    Provider<CheckInRepository>((ref) {
