...
class _CheckOutScreenState extends ConsumerState<CheckOutScreen> with SingleTickerProviderStateMixin {
  ...
  late final ProviderSubscription<CheckOutState> _checkOutListener;
  ...
  void initState() {
    ...
    _checkOutListener = ref.listenManual(checkOutNotifierProvider, (prev, next) {
      ...
      if (!next.isLoading && !controller.value.isTakingPicture) {
      ...
      if (next.isSuccess && mounted) {
        AppDialog.showSuccess(
          ...
          buttonText: 'Oke',
          onButtonPressed: () {
            ref.read(checkOutNotifierProvider.notifier).clear();
          },
        );
      } 
      else if (next.isError && next.errorMessage != null && mounted) {
        AppDialog.showError(
          context: context,
          title: 'Gagal',
  ...
  Widget build(BuildContext context) {
    ...
    final isLoading = ref.watch(checkOutNotifierProvider.select((s) => s.isLoading));
    ...
    final isInitialized = ref.watch(checkOutNotifierProvider.select((s) => s.isInitialized));
      ...
      child: Scaffold(
        ...
        body: Stack(
          children: [
            ...
            // Loading overlay
            if (isLoading) ...[
              AppLoading(message: 'Memproses check-in...'),
            ],
          ],
        ),
      ),
  ...
  Widget _buildSelfieSection(ScanQrEntity branchData) {
    final initializedState = ref.watch(checkOutNotifierProvider.select((s) => s.isInitialized));
  ...
  Widget _buildButton(ScanQrEntity branchData) {
    ...
    return _selfieImage == null ? GestureDetector(
      onTap: checkOutState.isLoading ? null : () async => await _onCapture(),
