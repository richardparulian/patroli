...
AppException exceptionFromFailure(Failure failure) {
  if (failure is NetworkFailure) {
    return NetworkException(message: failure.message);
  }

  if (failure is TimeoutFailure) {
    return TimeoutException(message: failure.message);
  }

  if (failure is UnauthorizedFailure || failure is AuthFailure) {
    return UnauthorizedException(message: failure.message);
  }

  if (failure is ValidationFailure || failure is InputFailure) {
    return BadRequestException(message: failure.message);
  }
